...
for j in range(len(Archive_filePath)):
    ...
    df2 = pd.read_csv(RTAC_filePath[k], encoding='cp1252', low_memory=False)
    df3 = pd.read_csv(RTAC_filePath[k+1], encoding='cp1252', low_memory=False)

    i = 0
    for i, row in df.iterrows():

        Archive_FreqArr.append(df.iloc[i, 0])
        print(Archive_FreqArr[i])
    ...
    for i, row in df3.iterrows():
        RTAC_FreqArr.append(df3.iloc[i, 1])
        # RTAC_SlewVal = df2.iloc[i, 2]
        ...
        # RTAC_SlewArr.append(float(RTAC_SlewVal))
        #
        RTAC_SeriesEventArr.append(df3.iloc[i, 3])
...
# ==================  OFFLINE EVENT DETECTION  ============================
    ...
    ArchiveSeriesEvents = offlineEventDetection(Archive_FreqArr)
...
# ================== PLOTTING ONLINE VS ONLINE FREQUENCY AND DETECTION TIMES =======================
    ...
    # Define RTAC frequency graph
    ax2.plot(RTAC_FreqArr, color='r',lw=1)
    ax2.set_ylim(59.8, 60.2)
    ax2.title.set_text('RTAC Data')
...
human_valid='Z:\Human Validation - 30.csv'        # human validation file containing expert assessment
